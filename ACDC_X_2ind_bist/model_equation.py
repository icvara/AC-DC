...
## with new distance equation with bistability asked, need to add back the transient time for oscillation...
...
def stable_distance(A,trr=tr_st,dt=dtt):
...
def oscillation_distance(A,trr=tr,dt=dtt):
        transient = int(trr/dt)
        max_list=argrelextrema(A[transient:], np.greater)
        maxValues=A[transient:][max_list]
             # for local minima
