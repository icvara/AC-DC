...
def Flow(X,Y,Z,ARA,par):
   # flow_x= 10**par['alpha_X'] +( np.power((10**par['beta_X']-10**par['alpha_X']) *ARA,par['n_ARAX'])) / ( np.power(10**par['K_ARAX'],par['n_ARAX']) + np.power(ARA,par['n_ARAX']))
    ...
    if maxrate_x < 0 :
       maxrate_x=0  
    flow_x = par['alpha_X'] + maxrate_x *(np.power(ARA,par['n_ARAX']))/( np.power(10**par['K_ARAX'],par['n_ARAX']) + np.power(ARA,par['n_ARAX']))
    ...
    if maxrate_y < 0 :
      maxrate_y=0  
    flow_y = par['alpha_Y'] +maxrate_y *( np.power(ARA,par['n_ARAY'])) / ( np.power(10**par['K_ARAY'],par['n_ARAY']) + np.power(ARA,par['n_ARAY']))
...
def distance(x,pars,totaltime=tt, dt=dtt, tr=tr ):
